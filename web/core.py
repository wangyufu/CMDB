...
class Asset(object):
    ...
    def get_asset_id_by_sn(self):
        ...
        if data:
            try:
                ...
                # only_check_sn=True 新资产
                if self.mandatory_check(data,
                                        only_check_sn=True):  # 如果资产信息以及存在数据，返回资产ID
    ...
    def save_new_asset_to_approval_zone(self):
        ...
        # 非defaults参数  只能使用 unique的字段 ，下面sn就是unique字段
        asset_already_in_approval_zone = models.NewAssetApprovalZone.objects.get_or_create(defaults={
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        ...
        try:
            ...
            if hasattr(component_obj, 'select_related'):  # this component is reverse m2m relation with Asset model
                objects_from_db = component_obj.select_related()    # select_related 跟 all是一样的
                for obj in objects_from_db:
                    key_field_data = getattr(obj, identify_field)   # 反射 取到关键字段
                    ...
                    if type(data_source) is list:
                        for source_data_item in data_source:  # 到客户端的数据 源里去找到跟 服务器数据库中key_field_data对应的条目
                            key_field_data_from_source_data = source_data_item.get(identify_field)  # 客户端数据的关键字段
                            if key_field_data_from_source_data: # 如果这个关键字段没有会在else下记录
                ...
                # model_obj_name 是 当前设备的表名字符串(NIC\RAM...)
    ...
    def __delete_components(self, all_components, delete_list, identify_field):
        ...
        # obj的关键字段在delete列表里就加上面的列表里，下面统一删除
    ...
    def __compare_componet(self, model_obj, fields_from_db, data_source):
        ...
        :param model_obj:  资产硬件(cpu\ram\disk)的一条
        :param fields_from_db: 对比字段
        ...
        for field in fields_from_db:
            ...
            if val_from_data_source:
                ...
                # 如果数据库这个字段的数据是xxx类型，就把客户端的数据转成该类型
